...
function patchPackageJson(json: any): any {
  ...
  json.name = '@notjustanna/rollem-language';
  json.dependencies = Object.fromEntries(
    Object.entries(json.dependencies).filter(([key]) => whitelistedDependencies.includes(key)),
  );
  json.contributors = [
    json.author,
    'Anna Silva <npm@notjustanna.net> (https://notjustanna.net)',
  ];
  delete json.author;
  json.homepage = 'https://github.com/notjustanna/rollem-language-patched';
  json.bugs = {
    url: 'https://github.com/notjustanna/rollem-language-patched/issues',
  }
  return json;
}
